...
S3Mock.prototype = {
	...
	headBucket: function(opts, callback) {
	  if (!opts || !opts.Bucket) {
	    throw new Error('Invalid requst, missing parameter Bucket!');
          }
	  var opts = _.extend({}, this.defaultOptions, applyBasePath(opts));
          if (fs.existsSync(opts.Bucket)) {
            // Bucket is valid
           callback(undefined,{});
          } else {
            // Bucket does not exist
            const err = Error(`Bucket ${opts.Bucket} does not exist!`);
            throw err;
          }
        },
